...
Once ALL files are written (or if no changes are needed), return a text summary to finish."""
    ...
    if hasattr(response, "tool_calls") and response.tool_calls:
        msg_log = f"Backend triggered tool: {response.tool_calls[0]['name']}"
    else:
        msg_log = f"Backend generated final text summary: {str(response.content)[:100].replace('\n', ' ')}..."
    
    return {
        "backend_messages": new_agent_messages, 
        "messages": [msg_log]
    }
