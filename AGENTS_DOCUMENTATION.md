...
*   **AI Model Configuration:** Uses `gemini-2.5-flash-lite` (with `max_retries=10`) with a temperature of `0` to ensure the breakdown is highly deterministic, logical, and formatted correctly as JSON without creative hallucinations.
...
*   **AI Model Configuration:** Uses `gemini-2.5-flash-lite` (with `max_retries=10`) with a temperature of `0.1`. A slight bump in temperature allows minimal creativity needed for writing clean code and solving logic problems.

---

## 3. Tester Agent (`src/agents/tester.py`)
*   **Role:** QA Automation Engineer
*   **Purpose:** Reads the code generated by the Backend Agent and automatically generates unit tests (e.g., via `pytest`). It identifies edge cases, bugs, or missing logic, and produces a `test_report.md` artifact.
*   **How it works:** Similar to other agents, it binds the `write_code_to_disk` tool to write both the test scripts and the test report artifact to the `workspace/tests/` directory. It then outputs a JSON summary that sets the `tester_done` flag and updates `test_report` and `review_feedback`.
*   **AI Model Configuration:** Uses `gemini-2.5-flash-lite` (with `max_retries=10`) with a temperature of `0` for strict correctness.

---

## 4. Tool Executor Nodes (`src/graph/workflow.py`)
*   **Role:** The Bridge (Environment Simulator)
*   **Purpose:** These are not AI models, but critical LangGraph execution nodes. We maintain isolated tool nodes (`backend_tools`, `frontend_tools`, and `tester_tools`) to ensure state isolation. When an agent outputs a tool call, the respective tool node catches it, runs the underlying Python function (`write_code_to_disk`), and returns the OS success/error string back to the specific agent's message history.

---

## 5. Reviewer Agent (`src/agents/reviewer.py`)
*   **Role:** QA Engineer / Code Reviewer
*   **Purpose:** Evaluates the generated files against the original requirements and the task lists to ensure the job is complete, functional, and bug-free. It specifically consumes the report generated by the Tester Agent.
...
*   **AI Model Configuration:** Uses `gemini-2.5-flash-lite` (with `max_retries=10`) with a temperature of `0` to ensure strict, ruthless, and logical evaluations.
...
*   **The Pipeline:**
    ...
    4.  Backend finishes and routes to the **Tester Agent**. Tester Agent loops with its **Tester Tool** node, then hits its marker node.
    5.  Frontend finishes and hits its marker node.
    6.  The **Join Node** acts as a synchronization barrier. It waits until both the Frontend and Tester branches report `done = True`.
    6.  Moves to **Reviewer**.
