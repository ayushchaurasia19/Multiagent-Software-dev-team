...
*   **Outputs & State Updates:**
    *   Writes physical files (e.g., `.py`, `.js`, `.html`, `.css`, `.json`) into the `workspace/` directory using the `write_code_to_disk` tool.
    *   Appends the interaction messages to `backend_messages` or `frontend_messages` state history lists.
*   **Output Format (LLM/Tool):**
    *   **During code generation:** Iteratively calls the tool `write_code_to_disk` with argument formats:
        *   `filename`: `string` (relative path to workspace)
...
*   **Outputs & State Updates:**
    *   Writes unit test suites (e.g. `tests/test_api.py`) and a markdown report (`tests/test_report.md`) to the `workspace/` directory using the `write_code_to_disk` tool.
    *   Appends messages to `tester_messages` and a status log to `messages`.
*   **Output Format (LLM/Tool):**
    *   **During test generation:** Calls the `write_code_to_disk` tool to output files.
...
*   **Purpose:** Non-AI utility nodes executing the `write_code_to_disk` tool calls request by the Backend, Frontend, or Tester agents.
*   **Outputs & State Updates:** 
    *   Appends a `ToolMessage` with the file write result (e.g., success or error status) back to the corresponding agent's message history.
